    ...
    simulateInstallments() {
        if (isPlatformBrowser(this.platformId)) {
            if (this.product.installmentNumber > 0 && this.product.installmentValue > 0) {
                this.product.installmentText = `em <span class="price">${this.product.installmentNumber}x</span> de R$ <span class="price">${this.product.installmentValue.toFixed(2).toString().replace('.', ',')}</span>`;
            }
        }
    }
}